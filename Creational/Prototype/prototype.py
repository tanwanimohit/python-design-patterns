...
that allows you to create new objects by copying an existing object, known as the prototype.
"""

from abc import ABC, abstractmethod

class DesktopPrototype(ABC):
    
    @abstractmethod
    def clone(self):
        raise NotImplementedError

class Desktop(DesktopPrototype):
    _motherboard = None
    _processors = None
    _memory = None
    _storage = None
    ...
    def clone(self):
        clone_desktop =  Desktop()
        clone_desktop.set_graphics_card(self.get_graphics_card())
        clone_desktop.set_motherboard(self.get_motherboard())
        clone_desktop.set_processors(self.get_processors())
        clone_desktop.set_memory(self.get_memory())
        clone_desktop.set_storage(self.get_storage())
        return clone_desktop
